var tinCongTacVienDuyetfn = {
    urlDefault: adm.urlDefault + '&act=loadPlug&rqPlug=projectMgr.plugin.backend.tinTuc.CongTacVien.DaDuyet.Class1, projectMgr.plugin.backend.tinTuc',
    setup: function () {
    },
    loadgrid: function () {
        ...
        $('#tinCongTacVienDuyetMdl-List').jqGrid({
            url: tinCongTacVienDuyetfn.urlDefault + '&subAct=get&Lang=' + Lang,
            height: 'auto',
            datatype: 'json',
            loadui: 'disable',
            ...
            multiselect: true,
            multiboxonly: true,
            pager: jQuery('#tinCongTacVienDuyetMdl-Pagerql'),
            onSelectRow: function (rowid) {
                tinCongTacVienDuyetfn.changeSubTin(rowid);
            },
            loadComplete: function () {
                ...
                adm.regType(typeof (tinfn), 'projectMgr.plugin.backend.tinTuc.Class1, projectMgr.plugin.backend.tinTuc', function () {
                    tinCongTacVienDuyetfn.loadTinList('');
                    tinCongTacVienDuyetfn.SubTinFn();
                });
                //adm.regQS(searchTxt, 'tinCongTacVienDuyetMdl-List');
                adm.watermarks(DMID, 'Gõ tên mục tin để lọc', function () {
                    if ($(searchTxt).val() == 'Tìm kiếm tin tức') {
                        $(searchTxt).val('');
                    }
                    tinCongTacVienDuyetfn.search();
                    if ($(searchTxt).val() == '') {
    ...
    loadTinList: function (_id) {
        $('#tinCongTacVienDuyetMdl-subTinMdl-List').jqGrid({
    ...
    yeuCauDuyet: function (st) {
        var s = '';
        s = jQuery("#tinCongTacVienDuyetMdl-List").jqGrid('getGridParam', 'selarrrow').toString();
        if (s == '') {
            ...
            for (j = 0; j < ss.length; j++) {
                var treedata = $("#tinCongTacVienDuyetMdl-List").jqGrid('getRowData', ss[j]);
                ll += ss[j] + ',';
            }
            $.ajax({
                url: tinCongTacVienDuyetfn.urlDefault + '&subAct=yeuCauDuyet&status=' + st,
                data: {
                    'ID': ll
                },
                success: function (dt) {
                    jQuery("#tinCongTacVienDuyetMdl-List").trigger('reloadGrid');
                }
            });
        }
    ...
    SubTinFn: function () {
        var T_ID = $('.mdl-head-searchTinCongTacVienDuyetMdl');
        var Btn = $('#tinCongTacVienDuyetMdl-subTinMdl-addBtn');
        ...
        $(Btn).unbind('click').click(function () {
            ...
            T_ID = $('.mdl-head-searchTinCongTacVienDuyetMdl');
            ...
            if (_v != '') {
                $.ajax({
                    ...
                    success: function () {
                        $(T_ID).val('');
                        $(T_ID).attr('_value', '');
                        $('#tinCongTacVienDuyetMdl-subTinMdl-List').trigger('reloadGrid');
                    }
                });
            }
        });
    ...
    delSubTin: function () {
        ...
        var s = '';
        s = jQuery("#tinCongTacVienDuyetMdl-subTinMdl-List").jqGrid('getGridParam', 'selarrrow').toString();
        if (s == '') {
    ...
    changeSubTin: function (_id) {
        var Btn = $('#tinCongTacVienDuyetMdl-subTinMdl-addBtn');
        $(Btn).attr('_id', _id);
        $('#tinCongTacVienDuyetMdl-subTinMdl-List').jqGrid('setGridParam', { url: tinfn.urlDefault + '&subAct=getSubTin&ID=' + _id }).trigger('reloadGrid');
    },
    add: function () {
        tinCongTacVienDuyetfn.loadHtml(function () {
            var newDlg = $('#tinCongTacVienDuyetMdl-dlgNew');
            $(newDlg).dialog({
                title: 'Thêm mới',
                width: 900,
                ...
                buttons: {
                    'Lưu': function () {
                        tinCongTacVienDuyetfn.save(false, function () {
                            tinCongTacVienDuyetfn.clearform();
                        });
                    },
                ...
                open: function () {
                    ...
                    adm.styleButton();
                    tinCongTacVienDuyetfn.clearform();
                    tinCongTacVienDuyetfn.addpopfn();
    ...
    edit: function () {
        var s = '';
        if (jQuery('#tinCongTacVienDuyetMdl-List').jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery('#tinCongTacVienDuyetMdl-List').jqGrid('getGridParam', 'selrow').toString();
        }
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                var treedata = $("#tinCongTacVienDuyetMdl-List").jqGrid('getRowData', s);
                ...
                tinCongTacVienDuyetfn.loadHtml(function () {
                    ...
                    $(newDlg).dialog({
                        ...
                        open: function () {
                            ...
                            tinCongTacVienDuyetfn.clearform();
                            $.ajax({
                                url: tinCongTacVienDuyetfn.urlDefault + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': __TIN_ID
                                },
                                success: function (dt) {
                                    ...
                                    $(lbladm_uploadTintuc_fileLists).find('a').click(function () {
                                        ...
                                        $.ajax({
                                            url: tinCongTacVienDuyetfn.urlDefault,
                                            data: {
                                                'subAct': 'DeleteDoc',
    ...
    del: function (fn) {
        ...
        if (s == '') {
            ...
            $.ajax({
                url: tinCongTacVienDuyetfn.urlDefault + '&subAct=del',
                dataType: 'script',
                data: {
                    'ID': ll
                },
    ...
    search: function () {
        ...
        timerSearch = setTimeout(function () {
            $('#tinCongTacVienDuyetMdl-List').jqGrid('setGridParam', { url: tinCongTacVienDuyetfn.urlDefault + "&subAct=get&q=" + q + "&DMID=" + dm }).trigger("reloadGrid");
        }, 500);
    },
    save: function (validate, fn) {
        ...
        $.ajax({
            url: tinCongTacVienDuyetfn.urlDefault + '&subAct=save',
            dataType: 'script',
            type: 'POST',
            data: {
                'ID': id,
    ...
    loadHtml: function (fn) {
        var dlg = $('#tinCongTacVienDuyetMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("projectMgr.plugin.backend.tinTuc.CongTacVien.DaDuyet.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
