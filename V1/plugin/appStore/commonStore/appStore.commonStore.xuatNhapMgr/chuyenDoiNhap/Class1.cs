...
[assembly: WebResource("appStore.commonStore.xuatNhapMgr.chuyenDoiNhap.htm.htm", "text/html", PerformSubstitution = true)]
[assembly: WebResource("appStore.commonStore.xuatNhapMgr.chuyenDoiNhap.JScript1.js", "text/javascript", PerformSubstitution = true)]
namespace appStore.commonStore.xuatNhapMgr.chuyenDoiNhap
{
            ...
            switch (subAct)
            {
                ...
                case "reports":
                    ...
                    if (!string.IsNullOrEmpty(ID))
                    {
                        ...
                        item.Ma = prefixMa + item.Ma;
                        item.XNCT = XuatNhapChiTietDal.SelectByXN_ID(item.ID.ToString());
                        var itemIn = Lib.GetResource(Assembly.GetExecutingAssembly(), "chuyenDoiNhap.in.htm");
                        var itemInRow = Lib.GetResource(Assembly.GetExecutingAssembly(), "chuyenDoiNhap.in-row.htm");
                ...
                case "scpt":
                    #region Nạp js
                    sb.AppendFormat(@"{0}"
                        , cs.GetWebResourceUrl(typeof(Class1), "appStore.commonStore.xuatNhapMgr.chuyenDoiNhap.JScript1.js"));
                    //sb.AppendFormat(@"{0}"
                    //    , cs.GetWebResourceUrl(typeof(Class1), "appStore.commonStore.xuatNhapMgr.chuyenDoiNhap.Publish.js"));
                    break;
                    #endregion
                default:
                    #region nạp
                    var listFn = FunctionDal.SelectByUserAndFNID(Security.Username, fnId);
                    sb.Append(Lib.GetResource(Assembly.GetExecutingAssembly(), "chuyenDoiNhap.mdl.htm"));                    
                    sb.AppendFormat(@"<script>$.getScript('{0}',function(){1});</script>"
                        , cs.GetWebResourceUrl(typeof(Class1), "appStore.commonStore.xuatNhapMgr.chuyenDoiNhap.JScript1.js")
                        , "{chuyenDoiNhapFn.loadgrid();}");
