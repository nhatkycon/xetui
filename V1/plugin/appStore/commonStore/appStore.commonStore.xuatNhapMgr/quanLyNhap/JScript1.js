var quanLyNhapFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=appStore.commonStore.xuatNhapMgr.quanLyNhap.Class1, appStore.commonStore.xuatNhapMgr'; },
    urlDefault1: function () { return adm.urlDefault1 + '&act=loadPlug&rqPlug=appStore.commonStore.xuatNhapMgr.quanLyNhap.Class1, appStore.commonStore.xuatNhapMgr'; },
    setup: function () { },
    loadgrid: function () {
        adm.styleButton();
        var DMID = $('.mdl-head-HangHoaFilterDanhMuc');
        var q = $('.mdl-head-search-hangHoaMgr');
        $('#quanLyNhapMdl-List').jqGrid({
            url: quanLyNhapFn.urlDefault().toString() + '&subAct=get',
            height: '400',
            datatype: 'json',
            loadui: 'disable',
            ...
            sortorder: 'desc',
            pager: jQuery('#quanLyNhapMdl-Pager'),
            rowNum: 10,
    ...
    XuatNhapChiTietItemFn: function (_obj) {
        ...
        Xoa.click(function () {
            ...
            obj.fadeOut('200');
            $.ajax({
                url: quanLyNhapFn.urlDefault().toString(),
                dataType: 'script',
                data: {
                    subAct: 'XoaXNChiTiet',
                    'ID': id
    ...
    recount: function () {
        var newDlg = $('#quanLyNhapMdl-dlgNew');
                ...
                if ($(cItem).length < 1) {
                    $.ajax({
                        ...
                        success: function (_dt) {
                            ...
                            quanLyNhapFn.XuatNhapChiTietItemFn(newItem);
                            quanLyNhapFn.recount();
                            quanLyNhapFn.updateXNCTItem(newItem);
                        }
                    });
                    HangHoa.focus();
                    HangHoa.val('');
                    ...
                    HangHoa.focus();
                    HangHoa.val('');
                    quanLyNhapFn.XuatNhapChiTietItemFn(cItem);
                    quanLyNhapFn.recount();
                    quanLyNhapFn.updateXNCTItem(cItem);
                }
                return false;
            },function (ul,item) {
                return $("<li></li>")
    ...
    refresh: function (grid) {
        if (typeof (grid) == 'undefined') grid == '#quanLyNhapMdl-List';
        var timerSearch;
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $(grid).jqGrid('setGridParam', { url: quanLyNhapFn.urlDefault().toString() + '&subAct=get' }).trigger('reloadGrid');
        }, 500);
    },
    edit: function (grid) {
        var s = '';
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                quanLyNhapFn.loadHtml(function () {
                    var newDlg = $('#quanLyNhapMdl-dlgNew');
                    $(newDlg).dialog({
                        title: 'Sửa',
                        width: 1010,
                        buttons: {
                            'Lưu': function () {
                                quanLyNhapFn.save(false, function () {
                                    quanLyNhapFn.clearform();
                                    quanLyNhapFn.draff(function (dt) {
                        ...
                        open: function () {
                            ...
                            quanLyNhapFn.clearform();
                            $.ajax({
                                url: quanLyNhapFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': s
                                },
    ...
    add: function (_newDlg, fn) {
        quanLyNhapFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        quanLyNhapFn.save(false, function (_ID) {
                            if (typeof (fn) == 'function') {
                                fn(_ID);
                            }
                            quanLyNhapFn.clearform();
                            quanLyNhapFn.draff(function (_dt) {
                                var dt = eval(_dt);
    ...
    loc: function () {
        quanLyNhapFn.loadHtml(function () {
            var newDlg = $('#quanLyNhapMdl-dlgFilter');
            $(newDlg).dialog({
                title: 'Lọc',
                width: 600,
                modal: true,
                buttons: {
                    'Tìm': function () {
                        ...
                        $('#quanLyNhapMdl-List').jqGrid('setGridParam', { url: quanLyNhapFn.urlDefault().toString() + '&subAct=get&DM_ID=' + _DM_ID + '&KH_ID=' + _KH_ID + '&TuNgay=' + _TuNgay + '&DenNgay=' + _DenNgay }).trigger('reloadGrid');

                    },
                    'Nhập lại': function () {
                        quanLyNhapFn.locClearform();
                    },
                    'Đóng': function () {
                        $(newDlg).dialog('close');
                    }
    ...
    del: function (grid) {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn xóa?')) {// Xác nhận xem có xóa hay không
                $.ajax({
                    url: quanLyNhapFn.urlDefault().toString() + '&subAct=del',
                    dataType: 'script',
                    data: { 'ID': s },
                    success: function (dt) {
    ...
    save: function (validate, fn, grid) {
        ...
        var err = '';
        if (_KH_ID == '') { err += 'Chọn nhà cung cấp<br/>'; };
    ...
    search: function () {
        ...
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $('#quanLyNhapMdl-List').jqGrid('setGridParam', { url: quanLyNhapFn.urlDefault().toString() + '&subAct=get&DM_ID=' + _DMID + '&q=' + q.val() }).trigger('reloadGrid');
        }, 500);
    },
    draff: function (fn) {
        $.ajax({
    ...
    createReport: function (id) {
        var request = document.location.href + quanLyNhapFn.urlDefault().toString() + '&subAct=reports&ID=' + id;
    ...
    loadHtml: function (fn) {
        var dlg = $('#quanLyNhapMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("appStore.commonStore.xuatNhapMgr.quanLyNhap.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
