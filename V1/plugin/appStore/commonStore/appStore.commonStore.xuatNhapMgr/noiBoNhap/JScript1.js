var noiBoNhapFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=appStore.commonStore.xuatNhapMgr.noiBoNhap.Class1, appStore.commonStore.xuatNhapMgr'; },
    urlDefault1: function () { return adm.urlDefault1 + '&act=loadPlug&rqPlug=appStore.commonStore.xuatNhapMgr.noiBoNhap.Class1, appStore.commonStore.xuatNhapMgr'; },
    setup: function () { },
    loadgrid: function () {
        adm.styleButton();
        var DMID = $('.mdl-head-HangHoaFilterDanhMuc');
        var q = $('.mdl-head-search-hangHoaMgr');
        $('#noiBoNhapMdl-List').jqGrid({
            url: noiBoNhapFn.urlDefault().toString() + '&subAct=get',
            height: '400',
            datatype: 'json',
            loadui: 'disable',
            ...
            sortorder: 'desc',
            pager: jQuery('#noiBoNhapMdl-Pager'),   
            rowNum: 10,
    ...
    XuatNhapChiTietItemFn: function (_obj) {
        ...
        Xoa.click(function () {
            ...
            obj.fadeOut('200');
            $.ajax({
                url: noiBoNhapFn.urlDefault().toString(),
                dataType: 'script',
                data: {
                    subAct: 'XoaXNChiTiet',
                    'ID': id
    ...
    recount: function () {
        var newDlg = $('#noiBoNhapMdl-dlgNew');
                ...
                if ($(cItem).length < 1) {
                    $.ajax({
                        ...
                        success: function (_dt) {
                            ...
                            noiBoNhapFn.XuatNhapChiTietItemFn(newItem);
                            noiBoNhapFn.recount();
                            noiBoNhapFn.updateXNCTItem(newItem);
                        }
                    });
                    HangHoa.focus();
                    HangHoa.val('');
                    ...
                    HangHoa.focus();
                    HangHoa.val('');
                    noiBoNhapFn.XuatNhapChiTietItemFn(cItem);
                    noiBoNhapFn.recount();
                    noiBoNhapFn.updateXNCTItem(cItem);
                }
                return false;
            });
    ...
    refresh: function (grid) {
        if (typeof (grid) == 'undefined') grid == '#noiBoNhapMdl-List';
        var timerSearch;
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $(grid).jqGrid('setGridParam', { url: noiBoNhapFn.urlDefault().toString() + '&subAct=get' }).trigger('reloadGrid');
        }, 500);
    },
    edit: function (grid) {
        var s = '';
            ...
            if (s.indexOf(',') != -1) {
                ...
                noiBoNhapFn.loadHtml(function () {
                    ...
                    $(newDlg).dialog({
                        ...
                        buttons: {
                            'Lưu': function () {
                                noiBoNhapFn.save(false, function () {
                                    noiBoNhapFn.clearform();
                                    noiBoNhapFn.draff(function (dt) {
                        ...
                        open: function () {
                            ...
                            noiBoNhapFn.clearform();
                            $.ajax({
                                url: noiBoNhapFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': s
                                },
    ...
    add: function (_newDlg, fn, obj) {
        noiBoNhapFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        noiBoNhapFn.save(false, function (_ID) {
                            if (typeof (fn) == 'function') {
                                fn(_ID);
                            }
                            noiBoNhapFn.clearform();
                            noiBoNhapFn.draff(function (_dt) {
                                var dt = eval(_dt);
    ...
    loc: function () {
        noiBoNhapFn.loadHtml(function () {
            var newDlg = $('#noiBoNhapMdl-dlgFilter');
            $(newDlg).dialog({
                title: 'Lọc',
                width: 600,
                modal: true,
                buttons: {
                    'Tìm': function () {
                        ...
                        $('#noiBoNhapMdl-List').jqGrid('setGridParam', { url: noiBoNhapFn.urlDefault().toString() + '&subAct=get&DM_ID=' + _DM_ID + '&KH_ID=' + _KH_ID + '&TuNgay=' + _TuNgay + '&DenNgay=' + _DenNgay }).trigger('reloadGrid');

                    },
                    'Nhập lại': function () {
                        noiBoNhapFn.locClearform();
                    },
                    'Đóng': function () {
                        $(newDlg).dialog('close');
                    }
    ...
    del: function (grid) {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn xóa?')) {// Xác nhận xem có xóa hay không
                $.ajax({
                    url: noiBoNhapFn.urlDefault().toString() + '&subAct=del',
                    dataType: 'script',
                    data: { 'ID': s },
                    success: function (dt) {
    ...
    search: function () {
        ...
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $('#noiBoNhapMdl-List').jqGrid('setGridParam', { url: noiBoNhapFn.urlDefault().toString() + '&subAct=get&DM_ID=' + _DMID + '&q=' + q.val() }).trigger('reloadGrid');
        }, 500);
    },
    draff: function (fn) {
        $.ajax({
    ...
    createReport: function (id) {
        var request = document.location.href + noiBoNhapFn.urlDefault().toString() + '&subAct=reports&ID=' + id;
    ...
    loadHtml: function (fn) {
        var dlg = $('#noiBoNhapMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("appStore.commonStore.xuatNhapMgr.noiBoNhap.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
