var quanLyNhapDauKyFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=appStore.commonStore.xuatNhapMgr.quanLyNhapDauKy.Class1, appStore.commonStore.xuatNhapMgr'; },
    urlDefault1: function () { return adm.urlDefault1 + '&act=loadPlug&rqPlug=appStore.commonStore.xuatNhapMgr.quanLyNhapDauKy.Class1, appStore.commonStore.xuatNhapMgr'; },
    setup: function () { },
    loadgrid: function () {
        adm.styleButton();
        var DMID = $('.mdl-head-HangHoaFilterDanhMuc');
        var q = $('.mdl-head-search-hangHoaMgr');
        $('#quanLyNhapDauKyMdl-List').jqGrid({
            url: quanLyNhapDauKyFn.urlDefault().toString() + '&subAct=get',
            height: '400',
            datatype: 'json',
            loadui: 'disable',
            ...
            colModel: [
                        ...
                        { name: 'XN_VAT', width: 10, resizable: true, sortable: true, align: 'right', hidden: true },
                        { name: 'XN_ChietKhau', width: 10, resizable: true, sortable: true, align: 'right', hidden: true },
                        { name: 'XN_CongTienHang', width: 10, resizable: true, sortable: true, align: 'right' },
                        { name: 'XN_ThanhToan', width: 10, resizable: true, sortable: true, align: 'right', hidden: true },
                        { name: 'XN_ConNo', width: 10, resizable: true, sortable: true, align: 'right', hidden: true },
            ...
            sortorder: 'desc',
            pager: jQuery('#quanLyNhapDauKyMdl-Pager'),
            rowNum: 10,
    ...
    XuatNhapChiTietItemFn: function (_obj) {
        ...
        Xoa.click(function () {
            ...
            obj.fadeOut('200');
            $.ajax({
                url: quanLyNhapDauKyFn.urlDefault().toString(),
                dataType: 'script',
                data: {
                    subAct: 'XoaXNChiTiet',
                    'ID': id
    ...
    recount: function () {
        var newDlg = $('#quanLyNhapDauKyMdl-dlgNew');
        ...
        var conNo = parseFloat(phaiTra) - parseFloat(thanhToan);
        PhaiTra.val(phaiTra);
        ConNo.val(conNo);
                ...
                if ($(cItem).length < 1) {
                    $.ajax({
                        ...
                        success: function (_dt) {
                            ...
                            quanLyNhapDauKyFn.XuatNhapChiTietItemFn(newItem);
                            quanLyNhapDauKyFn.recount();
                            quanLyNhapDauKyFn.updateXNCTItem(newItem);
                        }
                    });
                    HangHoa.focus();
                    HangHoa.val('');
                    ...
                    HangHoa.focus();
                    HangHoa.val('');
                    quanLyNhapDauKyFn.XuatNhapChiTietItemFn(cItem);
                    quanLyNhapDauKyFn.recount();
                    quanLyNhapDauKyFn.updateXNCTItem(cItem);
                }
                return false;
            },function (ul,item) {
                return $("<li></li>")
				            .data("item.autocomplete", item)
				            .append("<a><b>" + item.ma + '</b> ' + item.label + ' [' + adm.formatTienStr(item.giaNhap) + '] </a>')
				            .appendTo(ul);
            });
    ...
    refresh: function (grid) {
        if (typeof (grid) == 'undefined') grid == '#quanLyNhapDauKyMdl-List';
        var timerSearch;
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $(grid).jqGrid('setGridParam', { url: quanLyNhapDauKyFn.urlDefault().toString() + '&subAct=get' }).trigger('reloadGrid');
        }, 500);
    },
    edit: function (grid) {
        var s = '';
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                quanLyNhapDauKyFn.loadHtml(function () {
                    var newDlg = $('#quanLyNhapDauKyMdl-dlgNew');
                    $(newDlg).dialog({
                        title: 'Sửa',
                        width: 1010,
                        buttons: {
                            'Lưu': function () {
                                quanLyNhapDauKyFn.save(false, function () {
                                    quanLyNhapDauKyFn.clearform();
                                    quanLyNhapDauKyFn.draff(function (dt) {
                            ...
                            'In hóa đơn': function () {
                                quanLyNhapDauKyFn.createReport(s);
                            },
                        ...
                        open: function () {
                            ...
                            quanLyNhapDauKyFn.clearform();
                            $.ajax({
                                url: quanLyNhapDauKyFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': s
                                },
    ...
    add: function (_newDlg, fn) {
        quanLyNhapDauKyFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        quanLyNhapDauKyFn.save(false, function (_ID) {
                            if (typeof (fn) == 'function') {
                                fn(_ID);
                            }
                            quanLyNhapDauKyFn.clearform();
                            quanLyNhapDauKyFn.draff(function (_dt) {
                                var dt = eval(_dt);
                ...
                open: function () {
                    adm.styleButton();
                    quanLyNhapDauKyFn.clearform();
                    quanLyNhapDauKyFn.popfn(newDlg);
                    quanLyNhapDauKyFn.draff(function (_dt) {
                        var dt = eval(_dt);
                        var ID = newDlg.find('.ID');
                        var Ma = newDlg.find('.Ma');
    ...
    loc: function () {
        quanLyNhapDauKyFn.loadHtml(function () {
            var newDlg = $('#quanLyNhapDauKyMdl-dlgFilter');
            $(newDlg).dialog({
                title: 'Lọc',
                width: 600,
                modal: true,
                buttons: {
                    'Tìm': function () {
                        ...
                        $('#quanLyNhapDauKyMdl-List').jqGrid('setGridParam', { url: quanLyNhapDauKyFn.urlDefault().toString() + '&subAct=get&DM_ID=' + _DM_ID + '&KH_ID=' + _KH_ID + '&TuNgay=' + _TuNgay + '&DenNgay=' + _DenNgay }).trigger('reloadGrid');

                    },
                    'Nhập lại': function () {
                        quanLyNhapDauKyFn.locClearform();
                    },
                    'Đóng': function () {
                        $(newDlg).dialog('close');
                    }
                ...
                open: function () {
                    adm.styleButton();
                    quanLyNhapDauKyFn.locClearform();
                    quanLyNhapDauKyFn.locPopfn(newDlg);
                }
            });
        });
    },
    ...
    del: function (grid) {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn xóa?')) {// Xác nhận xem có xóa hay không
                $.ajax({
                    url: quanLyNhapDauKyFn.urlDefault().toString() + '&subAct=del',
                    dataType: 'script',
                    data: { 'ID': s },
                    success: function (dt) {
    ...
    search: function () {
        ...
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $('#quanLyNhapDauKyMdl-List').jqGrid('setGridParam', { url: quanLyNhapDauKyFn.urlDefault().toString() + '&subAct=get&DM_ID=' + _DMID + '&q=' + q.val() }).trigger('reloadGrid');
        }, 500);
    },
    draff: function (fn) {
        $.ajax({
    ...
    createReport: function (id) {
        var request = document.location.href + quanLyNhapDauKyFn.urlDefault().toString() + '&subAct=reports&ID=' + id;
    ...
    loadHtml: function (fn) {
        var dlg = $('#quanLyNhapDauKyMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("appStore.commonStore.xuatNhapMgr.quanLyNhapDauKy.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
