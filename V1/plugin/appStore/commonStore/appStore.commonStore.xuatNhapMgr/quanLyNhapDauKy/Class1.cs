...
[assembly: WebResource("appStore.commonStore.xuatNhapMgr.quanLyNhapDauKy.htm.htm", "text/html", PerformSubstitution = true)]
[assembly: WebResource("appStore.commonStore.xuatNhapMgr.quanLyNhapDauKy.JScript1.js", "text/javascript", PerformSubstitution = true)]
namespace appStore.commonStore.xuatNhapMgr.quanLyNhapDauKy
{
    ...
    public class Class1 : docPlugUI
    {
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            var danhMucLoaiXuatNhap = DanhMucDal.SelectByMa("LXN-NDK");
            var isXuat = false;
            #endregion
            switch (subAct)
            {
                case "get":
                    ...
                    var pagerGet = XuatNhapDal.pagerXuatNhap(false.ToString(), true.ToString(), false.ToString(),
                        false.ToString(), null,
                        jgrsidx + " " + jgrsord, _q, Convert.ToInt32(jgRows));
                ...
                case "save":
                    ...
                    if (Security.IsAuthenticated())
                    {
                        ...
                        if (string.IsNullOrEmpty(ThanhToan)) ThanhToan = "0";
                        item.DauKy = true;
                ...
                case "reports":
                    ...
                    if (!string.IsNullOrEmpty(ID))
                    {
                        ...
                        item.Ma = prefixMa + item.Ma;
                        item.XNCT = XuatNhapChiTietDal.SelectByXN_ID(item.ID.ToString());
                        var itemIn = Lib.GetResource(Assembly.GetExecutingAssembly(), "quanLyNhapDauKy.in.htm");
                        var itemInRow = Lib.GetResource(Assembly.GetExecutingAssembly(), "quanLyNhapDauKy.in-row.htm");
                ...
                case "scpt":
                    #region Nạp js
                    sb.AppendFormat(@"{0}"
                        , cs.GetWebResourceUrl(typeof(Class1), "appStore.commonStore.xuatNhapMgr.quanLyNhapDauKy.JScript1.js"));
                    //sb.AppendFormat(@"{0}"
                    //    , cs.GetWebResourceUrl(typeof(Class1), "appStore.commonStore.xuatNhapMgr.quanLyNhapDauKy.Publish.js"));
                    break;
                    #endregion
                default:
                    #region nạp
                    var listFn = FunctionDal.SelectByUserAndFNID(Security.Username, fnId);
                    sb.Append(Lib.GetResource(Assembly.GetExecutingAssembly(), "quanLyNhapDauKy.mdl.htm"));                    
                    sb.AppendFormat(@"<script>$.getScript('{0}',function(){1});</script>"
                        , cs.GetWebResourceUrl(typeof(Class1), "appStore.commonStore.xuatNhapMgr.quanLyNhapDauKy.JScript1.js")
                        , "{quanLyNhapDauKyFn.loadgrid();}");
