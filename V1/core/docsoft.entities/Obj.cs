...
namespace docsoft.entities
{
    ...
    #endregion
    #region Redis
    public class ObjRedis
    {
        const string ItemKey = "urn:obj:{0}";
        const string ListKey = "urn:obj:list:{0}";
        private readonly IRedisClient _redisClient;
        public ObjRedis(IRedisClient client)
        {
            _redisClient = client;
        }
        public Obj GetById(Guid id)
        {
            var key = string.Format(ItemKey, id);
            var item = _redisClient.Get<Obj>(key);
            if (item == null)
            {
                item = ObjDal.SelectById(id);
                _redisClient.Set(key, item);
            }
            return item;
        }
        public Obj GetByAlias(string alias)
        {
