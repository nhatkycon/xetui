...
[assembly: WebResource("appStore.commonStore.thuChiMgr.baoCaoDoanhSo.htm.htm", "text/html", PerformSubstitution = true)]
[assembly: WebResource("appStore.commonStore.thuChiMgr.baoCaoDoanhSo.JScript1.js", "text/javascript", PerformSubstitution = true)]
namespace appStore.commonStore.thuChiMgr.baoCaoDoanhSo
{
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            switch (subAct)
            {
                case "get":
                    ...
                    var pagerGet = TuVanDichVuDal.BaoCaoDoanhSoTuNgayDenNgay(DAL.con(), TuNgay, DenNgay, null);
                        ...
                        ListRow.Add(new jgridRow(item.ID.ToString(), new string[]
                                                                         {
                                                                             ...
                                                                             , item._KhachHang.Ten
                                                                             , item._DichVu.Ten
                                                                             , item.LoaiQuy == 0 ? item.Gia.ToString() : ""
                                                                             , item.LoaiQuy == 0 ? item.ConNo.ToString() : ""
                                                                             , item.LoaiQuy != 0 ? item.Gia.ToString() : ""
                                                                             , item.LoaiQuy != 0 ? item.ConNo.ToString() : ""
                                                                             , item.NhanVien_Ten
                                                                         }));
                ...
                case "scpt":
                    #region Nạp js
                    sb.AppendFormat(@"{0}"
                        , cs.GetWebResourceUrl(typeof(Class1), "appStore.commonStore.thuChiMgr.baoCaoDoanhSo.JScript1.js"));
                    //sb.AppendFormat(@"{0}"
                    //    , cs.GetWebResourceUrl(typeof(Class1), "appStore.commonStore.thuChiMgr.baoCaoDoanhSo.Publish.js"));
                    break;
                    #endregion
                default:
                    #region nạp
                    var listFn = FunctionDal.SelectByUserAndFNID(Security.Username, fnId);
                    sb.Append(Lib.GetResource(Assembly.GetExecutingAssembly(), "baoCaoDoanhSo.mdl.htm"));                    
                    sb.AppendFormat(@"<script>$.getScript('{0}',function(){1});</script>"
                        , cs.GetWebResourceUrl(typeof(Class1), "appStore.commonStore.thuChiMgr.baoCaoDoanhSo.JScript1.js")
                        , "{baoCaoDoanhSoFn.loadgrid();}");
