var chuyenDoiNhapFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=appStore.commonStore.xuatNhapMgr.chuyenDoiNhap.Class1, appStore.commonStore.xuatNhapMgr'; },
    urlDefault1: function () { return adm.urlDefault1 + '&act=loadPlug&rqPlug=appStore.commonStore.xuatNhapMgr.chuyenDoiNhap.Class1, appStore.commonStore.xuatNhapMgr'; },
    setup: function () { },
    loadgrid: function () {
        adm.styleButton();
        var DMID = $('.mdl-head-HangHoaFilterDanhMuc');
        var q = $('.mdl-head-search-hangHoaMgr');
        $('#chuyenDoiNhapMdl-List').jqGrid({
            url: chuyenDoiNhapFn.urlDefault().toString() + '&subAct=get',
            height: '400',
            datatype: 'json',
            loadui: 'disable',
            ...
            sortorder: 'desc',
            pager: jQuery('#chuyenDoiNhapMdl-Pager'),   
            rowNum: 10,
    ...
    XuatNhapChiTietItemFn: function (_obj) {
        ...
        Xoa.click(function () {
            ...
            obj.fadeOut('200');
            $.ajax({
                url: chuyenDoiNhapFn.urlDefault().toString(),
                dataType: 'script',
                data: {
                    subAct: 'XoaXNChiTiet',
                    'ID': id
    ...
    recount: function () {
        var newDlg = $('#chuyenDoiNhapMdl-dlgNew');
                ...
                if ($(cItem).length < 1) {
                    $.ajax({
                        ...
                        success: function (_dt) {
                            ...
                            chuyenDoiNhapFn.XuatNhapChiTietItemFn(newItem);
                            chuyenDoiNhapFn.recount();
                            chuyenDoiNhapFn.updateXNCTItem(newItem);
                        }
                    });
                    HangHoa.focus();
                    HangHoa.val('');
                    ...
                    HangHoa.focus();
                    HangHoa.val('');
                    chuyenDoiNhapFn.XuatNhapChiTietItemFn(cItem);
                    chuyenDoiNhapFn.recount();
                    chuyenDoiNhapFn.updateXNCTItem(cItem);
                }
                return false;
            }, function (ul, item) {
                return $("<li></li>")
    ...
    refresh: function (grid) {
        if (typeof (grid) == 'undefined') grid == '#chuyenDoiNhapMdl-List';
        var timerSearch;
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $(grid).jqGrid('setGridParam', { url: chuyenDoiNhapFn.urlDefault().toString() + '&subAct=get' }).trigger('reloadGrid');
        }, 500);
    },
    edit: function (grid) {
        var s = '';
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                chuyenDoiNhapFn.loadHtml(function () {
                    var newDlg = $('#chuyenDoiNhapMdl-dlgNew');
                    $(newDlg).dialog({
                        title: 'Sửa',
                        width: 1010,
                        buttons: {
                            'Lưu': function () {
                                chuyenDoiNhapFn.save(false, function () {
                                    chuyenDoiNhapFn.clearform();
                                    chuyenDoiNhapFn.draff(function (dt) {
                            ...
                            'In hóa đơn': function () {
                                chuyenDoiNhapFn.createReport(s);
                            },
                        ...
                        open: function () {
                            ...
                            chuyenDoiNhapFn.clearform();
                            $.ajax({
                                url: chuyenDoiNhapFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': s
                                },
    ...
    add: function (_newDlg, fn, obj) {
        chuyenDoiNhapFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        chuyenDoiNhapFn.save(false, function (_ID) {
                            if (typeof (fn) == 'function') {
                                fn(_ID);
                            }
                            chuyenDoiNhapFn.clearform();
                            chuyenDoiNhapFn.draff(function (_dt) {
                                var dt = eval(_dt);
    ...
    loc: function () {
        chuyenDoiNhapFn.loadHtml(function () {
            var newDlg = $('#chuyenDoiNhapMdl-dlgFilter');
            $(newDlg).dialog({
                title: 'Lọc',
                width: 600,
                modal: true,
                buttons: {
                    'Tìm': function () {
                        ...
                        $('#chuyenDoiNhapMdl-List').jqGrid('setGridParam', { url: chuyenDoiNhapFn.urlDefault().toString() + '&subAct=get&DM_ID=' + _DM_ID + '&KH_ID=' + _KH_ID + '&TuNgay=' + _TuNgay + '&DenNgay=' + _DenNgay }).trigger('reloadGrid');

                    },
                    'Nhập lại': function () {
                        chuyenDoiNhapFn.locClearform();
                    },
                    'Đóng': function () {
                        $(newDlg).dialog('close');
                    }
                ...
                open: function () {
                    adm.styleButton();
                    chuyenDoiNhapFn.locClearform();
                    chuyenDoiNhapFn.locPopfn(newDlg);
                }
            });
        });
    },
    ...
    del: function (grid) {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn xóa?')) {// Xác nhận xem có xóa hay không
                $.ajax({
                    url: chuyenDoiNhapFn.urlDefault().toString() + '&subAct=del',
                    dataType: 'script',
                    data: { 'ID': s },
                    success: function (dt) {
    ...
    search: function () {
        ...
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $('#chuyenDoiNhapMdl-List').jqGrid('setGridParam', { url: chuyenDoiNhapFn.urlDefault().toString() + '&subAct=get&DM_ID=' + _DMID + '&q=' + q.val() }).trigger('reloadGrid');
        }, 500);
    },
    draff: function (fn) {
        $.ajax({
    ...
    createReport: function (id) {
        var request = document.location.href + chuyenDoiNhapFn.urlDefault().toString() + '&subAct=reports&ID=' + id;
    ...
    loadHtml: function (fn) {
        var dlg = $('#chuyenDoiNhapMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("appStore.commonStore.xuatNhapMgr.chuyenDoiNhap.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
