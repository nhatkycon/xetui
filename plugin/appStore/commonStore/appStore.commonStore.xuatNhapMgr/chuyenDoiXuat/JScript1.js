var chuyenDoiXuatFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=appStore.commonStore.xuatNhapMgr.chuyenDoiXuat.Class1, appStore.commonStore.xuatNhapMgr'; },
    urlDefault1: function () { return adm.urlDefault1 + '&act=loadPlug&rqPlug=appStore.commonStore.xuatNhapMgr.chuyenDoiXuat.Class1, appStore.commonStore.xuatNhapMgr'; },
    setup: function () { },
    loadgrid: function () {
        adm.styleButton();
        var DMID = $('.mdl-head-HangHoaFilterDanhMuc');
        var q = $('.mdl-head-search-hangHoaMgr');
        $('#chuyenDoiXuatMdl-List').jqGrid({
            url: chuyenDoiXuatFn.urlDefault().toString() + '&subAct=get',
            height: '400',
            datatype: 'json',
            loadui: 'disable',
            ...
            sortorder: 'desc',
            pager: jQuery('#chuyenDoiXuatMdl-Pager'),   
            rowNum: 10,
    ...
    XuatNhapChiTietItemFn: function (_obj) {
        ...
        Xoa.click(function () {
            ...
            obj.fadeOut('200');
            $.ajax({
                url: chuyenDoiXuatFn.urlDefault().toString(),
                dataType: 'script',
                data: {
                    subAct: 'XoaXNChiTiet',
                    'ID': id
    ...
    recount: function () {
        var newDlg = $('#chuyenDoiXuatMdl-dlgNew');
                ...
                if ($(cItem).length < 1) {
                    $.ajax({
                        ...
                        success: function (_dt) {
                            ...
                            chuyenDoiXuatFn.XuatNhapChiTietItemFn(newItem);
                            chuyenDoiXuatFn.recount();
                            chuyenDoiXuatFn.updateXNCTItem(newItem);
                        }
                    });
                    HangHoa.focus();
                    HangHoa.val('');
                    ...
                    HangHoa.focus();
                    HangHoa.val('');
                    chuyenDoiXuatFn.XuatNhapChiTietItemFn(cItem);
                    chuyenDoiXuatFn.recount();
                    chuyenDoiXuatFn.updateXNCTItem(cItem);
                }
                return false;
            }, function (ul, item) {
                return $("<li></li>")
    ...
    refresh: function (grid) {
        if (typeof (grid) == 'undefined') grid == '#chuyenDoiXuatMdl-List';
        var timerSearch;
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $(grid).jqGrid('setGridParam', { url: chuyenDoiXuatFn.urlDefault().toString() + '&subAct=get' }).trigger('reloadGrid');
        }, 500);
    },
    edit: function (grid) {
        var s = '';
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                chuyenDoiXuatFn.loadHtml(function () {
                    var newDlg = $('#chuyenDoiXuatMdl-dlgNew');
                    $(newDlg).dialog({
                        title: 'Sửa',
                        width: 1010,
                        buttons: {
                            'Lưu': function () {
                                chuyenDoiXuatFn.save(false, function () {
                                    chuyenDoiXuatFn.clearform();
                                    chuyenDoiXuatFn.draff(function (dt) {
                            ...
                            'In hóa đơn': function () {
                                chuyenDoiXuatFn.createReport(s);
                            },
                        ...
                        open: function () {
                            ...
                            chuyenDoiXuatFn.clearform();
                            $.ajax({
                                url: chuyenDoiXuatFn.urlDefault().toString() + '&subAct=edit',
                                dataType: 'script',
                                data: {
                                    'ID': s
                                },
    ...
    add: function (_newDlg, fn, obj) {
        chuyenDoiXuatFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        chuyenDoiXuatFn.save(false, function (_ID) {
                            if (typeof (fn) == 'function') {
                                fn(_ID);
                            }
                            chuyenDoiXuatFn.clearform();
                            chuyenDoiXuatFn.draff(function (_dt) {
                                var dt = eval(_dt);
    ...
    loc: function () {
        chuyenDoiXuatFn.loadHtml(function () {
            var newDlg = $('#chuyenDoiXuatMdl-dlgFilter');
            $(newDlg).dialog({
                title: 'Lọc',
                width: 600,
                modal: true,
                buttons: {
                    'Tìm': function () {
                        ...
                        $('#chuyenDoiXuatMdl-List').jqGrid('setGridParam', { url: chuyenDoiXuatFn.urlDefault().toString() + '&subAct=get&DM_ID=' + _DM_ID + '&KH_ID=' + _KH_ID + '&TuNgay=' + _TuNgay + '&DenNgay=' + _DenNgay }).trigger('reloadGrid');

                    },
                    'Nhập lại': function () {
                        chuyenDoiXuatFn.locClearform();
                    },
                    'Đóng': function () {
                        $(newDlg).dialog('close');
                    }
                ...
                open: function () {
                    adm.styleButton();
                    chuyenDoiXuatFn.locClearform();
                    chuyenDoiXuatFn.locPopfn(newDlg);
                }
            });
        });
    },
    ...
    del: function (grid) {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn xóa?')) {// Xác nhận xem có xóa hay không
                $.ajax({
                    url: chuyenDoiXuatFn.urlDefault().toString() + '&subAct=del',
                    dataType: 'script',
                    data: { 'ID': s },
                    success: function (dt) {
    ...
    search: function () {
        ...
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $('#chuyenDoiXuatMdl-List').jqGrid('setGridParam', { url: chuyenDoiXuatFn.urlDefault().toString() + '&subAct=get&DM_ID=' + _DMID + '&q=' + q.val() }).trigger('reloadGrid');
        }, 500);
    },
    draff: function (fn) {
        $.ajax({
    ...
    createReport: function (id) {
        var request = document.location.href + chuyenDoiXuatFn.urlDefault().toString() + '&subAct=reports&ID=' + id;
    ...
    loadHtml: function (fn) {
        var dlg = $('#chuyenDoiXuatMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("appStore.commonStore.xuatNhapMgr.chuyenDoiXuat.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
