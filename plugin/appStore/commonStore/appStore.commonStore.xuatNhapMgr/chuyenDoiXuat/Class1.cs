...
[assembly: WebResource("appStore.commonStore.xuatNhapMgr.chuyenDoiXuat.htm.htm", "text/html", PerformSubstitution = true)]
[assembly: WebResource("appStore.commonStore.xuatNhapMgr.chuyenDoiXuat.JScript1.js", "text/javascript", PerformSubstitution = true)]
namespace appStore.commonStore.xuatNhapMgr.chuyenDoiXuat
{
            ...
            switch (subAct)
            {
                ...
                case "reports":
                    ...
                    if (!string.IsNullOrEmpty(ID))
                    {
                        ...
                        item.Ma = prefixMa + item.Ma;
                        item.XNCT = XuatNhapChiTietDal.SelectByXN_ID(item.ID.ToString());
                        var itemIn = Lib.GetResource(Assembly.GetExecutingAssembly(), "chuyenDoiXuat.in.htm");
                        var itemInRow = Lib.GetResource(Assembly.GetExecutingAssembly(), "chuyenDoiXuat.in-row.htm");
                ...
                case "scpt":
                    #region Nạp js
                    sb.AppendFormat(@"{0}"
                        , cs.GetWebResourceUrl(typeof(Class1), "appStore.commonStore.xuatNhapMgr.chuyenDoiXuat.JScript1.js"));
                    //sb.AppendFormat(@"{0}"
                    //    , cs.GetWebResourceUrl(typeof(Class1), "appStore.commonStore.xuatNhapMgr.chuyenDoiXuat.Publish.js"));
                    break;
                    #endregion
                default:
                    #region nạp
                    var listFn = FunctionDal.SelectByUserAndFNID(Security.Username, fnId);
                    sb.Append(Lib.GetResource(Assembly.GetExecutingAssembly(), "chuyenDoiXuat.mdl.htm"));                    
                    sb.AppendFormat(@"<script>$.getScript('{0}',function(){1});</script>"
                        , cs.GetWebResourceUrl(typeof(Class1), "appStore.commonStore.xuatNhapMgr.chuyenDoiXuat.JScript1.js")
                        , "{chuyenDoiXuatFn.loadgrid();}");
