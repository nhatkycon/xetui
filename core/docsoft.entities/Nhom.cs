...
namespace docsoft.entities
{
    ...
    #endregion
    #region Redis
    public class NhomRedis
    {
        const string ItemKey = "urn:nhom:{0}";
        const string ListKey = "urn:nhom:list:{0}";
        private readonly IRedisClient _redisClient;
        public NhomRedis(IRedisClient client)
        {
            _redisClient = client;
        }
        ...
        public Nhom GetById(int id)
        {
            ...
            var item = _redisClient.Get<Nhom>(key);
            if (item == null)
            {
                item = NhomDal.SelectById(id);
                _redisClient.Set(key, item);
            }
            return item;
        }
        public Nhom GetByRowId(Guid rowid)
        {
            var idKey = string.Format(ItemKey, rowid);
            var id = _redisClient.Get<int>(idKey);
            if (id == 0) return null;
            var item = GetById(id);
            return item;
        }
        ...
        public List<Nhom> GetAllItems(int start, int stop)
        {
            var ids = GetAll().GetRange(start, stop).ToList();
            return ids.Select(id => GetById(Convert.ToInt32(id))).ToList();
        }
        ...
        public List<Nhom> GetTopItems(int start, int stop)
        {
            var ids = GetTop().GetRange(start, stop).ToList();
            return ids.Select(id => GetById(Convert.ToInt32(id))).ToList();
        }
        public IRedisList GetChuaDuyet()
        {
            var key = string.Format(ListKey, "chuaDuyet");
            var ids = _redisClient.Lists[key];
            return ids;
        }
        public List<Nhom> GetChuaDuyetItems(int start, int stop)
        {
            var ids = GetChuaDuyet().GetRange(start, stop).ToList();
            return ids.Select(id => GetById(Convert.ToInt32(id))).ToList();
        }
    }
    #endregion
